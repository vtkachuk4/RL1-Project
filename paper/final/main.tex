...
% the neural network is trained by performing a gradient descent on the loss function.
...
% Further, we noticed that the performance with $\tanh$ for all the tiling bounds is even better than it was without it for $u=100$.
% This might indicate that the optimal tiling bound is not $u=100$ and $\tanh$ was able to apply the appropriate normalization to find it.
...
% We will contact the original author to address any implementation differences, but further investigation into the plausible causes and further experimentation are required.
...
% [************NEED RESULTS SECTION TO REFLECT THIS. TANH + OTHERS, TANH WITH DIFFERENT BOUNDS, AND FTA + TANH IN CARTPOLE************]. 
...
% and more to evaluate FTA distributions in other envs, (more thorough robustness evaluation)
